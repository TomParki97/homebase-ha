"""Constants for HomeBase."""

DOMAIN = "homebase"

SIGNAL_CHORE_REMINDER = f"{DOMAIN}_chore_reminder"

CONF_DUE_REMINDERS_ENABLED = "due_reminders_enabled"
CONF_OVERDUE_REMINDERS_ENABLED = "overdue_reminders_enabled"
CONF_REMINDER_ACTION = "reminder_action"

DEFAULT_DUE_REMINDERS_ENABLED = True
DEFAULT_OVERDUE_REMINDERS_ENABLED = True

STORAGE_KEY = f"{DOMAIN}.chores"
STORAGE_VERSION = 1

MAINTENANCE_STORAGE_KEY = f"{DOMAIN}.maintenance"
MAINTENANCE_STORAGE_VERSION = 1
SIGNAL_MAINTENANCE_REMINDER = f"{DOMAIN}_maintenance_reminder"

CONF_MAINTENANCE_DUE_REMINDERS_ENABLED = (
    "maintenance_due_reminders_enabled"
)
CONF_MAINTENANCE_OVERDUE_REMINDERS_ENABLED = (
    "maintenance_overdue_reminders_enabled"
)
CONF_MAINTENANCE_REMINDER_ACTION = "maintenance_reminder_action"

DEFAULT_MAINTENANCE_DUE_REMINDERS_ENABLED = True
DEFAULT_MAINTENANCE_OVERDUE_REMINDERS_ENABLED = True
